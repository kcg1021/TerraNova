    ...
    <header className="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/60 dark:border-slate-700/60 px-3 md:px-6 h-12 md:h-14 flex items-center justify-between shadow-sm">
      ...
      {isAdminRoute ? (
        ...
        <Link to="/" className="flex items-center gap-2 md:gap-3 no-underline group min-w-0">
          ...
          {config.title ? (
            <span className="text-sm md:text-lg font-bold text-slate-900 dark:text-white tracking-tight truncate">
              {config.title}
            </span>
          ) : config.slogan ? (
      ...
      <div className="flex items-center gap-1 md:gap-2">
        ...
        <div className="relative" ref={themeMenuRef}>
          <button
            ...
            className="p-2 rounded-lg text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all duration-150 cursor-pointer"
          ...
          {themeMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-40 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg shadow-lg shadow-black/5 dark:shadow-black/30 z-50 py-1 overflow-hidden">
        ...
        {user && (
          <div className="relative" ref={menuRef}>
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg border whitespace-nowrap transition-all duration-150 cursor-pointer ${
                userMenuOpen
                  ...
                  : 'text-slate-700 dark:text-slate-200 bg-transparent border-slate-200 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
            ...
            {userMenuOpen && (
              <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg shadow-black/5 dark:shadow-black/40 z-50 overflow-hidden">
                {/* 사용자 정보 헤더 */}
                ...
                <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-xs text-slate-400 dark:text-slate-400 uppercase tracking-wide">세션 남은 시간</div>
                      <div className="text-sm font-semibold text-slate-900 dark:text-white mt-0.5 tabular-nums">{remainingTime}</div>
                    </div>
                    <button
...
function AdminBreadcrumb() {
  ...
  const Separator = () => (
    <Icon name="chevronRight" className="w-3.5 h-3.5 text-slate-300 dark:text-slate-600 flex-shrink-0" />
  )
  ...
  return (
    <nav className="flex items-center gap-1.5 min-w-0 text-sm">
      <Link
        to="/"
        className="flex items-center gap-1.5 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 transition-colors flex-shrink-0"
      >
